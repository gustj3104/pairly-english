...
- `GET /api/v1/dictionary/lookup?word=robot` returns a pronunciation (nullable), 1-5
  `koreanTranslations`, and 1-3 `meanings` (each `senseId` + `partOfSpeech` + `definition` +
  `example`) — all generated by a **single** Mindlogic call. Add `&retry=true` to force a fresh
  attempt (see "Automatic-retry cooldown" below). Limited to 60 requests/hour/IP.
...
- `PUT /api/v1/vocabulary/:normalizedWord` accepts `senseId` and optional paired `articleId` /
  `contextSentence`. Definition/example/koreanTranslations are copied from the canonical server
  cache, never the client.
- `DELETE /api/v1/vocabulary/:normalizedWord` is idempotent and returns `204`.

### Single-call AI lookup (replaces FreeDictionaryAPI + separate Korean translation)

Every dictionary lookup used to be two calls: an English definition from FreeDictionaryAPI.com
(Wiktionary data, CC BY-SA 4.0, with a dictionaryapi.dev fallback), then a **separate** Mindlogic
call asking only for a Korean gloss of that definition. This had two production problems: (1)
Wiktionary's sense ordering is etymological, not "current usage" — e.g. looking up `robot`
...
`koreanTranslations`, and up to three meanings together, instructed to prioritize the sense
"currently in common use" over a historical/etymological one. FreeDictionaryAPI,
dictionaryapi.dev, and every HTTP call to either are gone — `src/services/dictionary/provider.ts`,
`secondary-provider.ts`, `fallback.ts`, `provider-http.ts`, and the translation-only
`translation.ts` were all deleted; `DictionaryService` no longer takes a `fetch` implementation at
all. There is no English-only fallback: an AI failure fails the whole lookup (a Retry action, not
a silently-degraded "unavailable" Korean section).

Structured JSON contract (`DICTIONARY_LOOKUP_RESPONSE_FORMAT`, strict mode):
...
`pronunciation` is the empty string when the model isn't confident (normalized to `null` by
`dictionaryLookupAiResponseSchema`) rather than a nullable JSON Schema type, since this gateway's
`strict: true` structured-output support for nullable field types is unverified.
`koreanTranslations` must be non-empty (1-5 items, deduplicated, must contain Hangul, no HTML/
URL/Markdown/control characters); `meanings` must have 1-3 entries, each with a real part of
speech, an English definition, and a short natural example sentence. Any malformed JSON or schema
violation — including an empty `koreanTranslations` array — is rejected outright
(`DICTIONARY_INVALID_RESPONSE`), never silently trimmed into a partial "success."
...
unlike `gpt-5.4-mini`'s rate (set after a real smoke test), no real chat completion call against
`gpt-5.6-luna` has been made from this codebase yet. The registered rate
(`inputCreditsPerThousandTokens: 0.8`, `outputCreditsPerThousandTokens: 4.5`) is an **operator-
supplied placeholder**, copied from the nearest already-accepted reference point in the table
(`gpt-5.4-mini`, the other "mini/economical" tier model) rather than invented from scratch —
확인 불가. Treat it as provisional: reconcile against `GET /credits/` after the first real calls
land in production and correct the table (plus re-run a one-shot smoke test per "Procedure for a
one-time real smoke test" below) once Mindlogic's actual rate for this model is confirmed.

**Feature name kept as `dictionary_translation`.** `credit_feature` is a Postgres enum
(`src/db/schema.ts`); adding a new value (e.g. `dictionary_lookup`) requires a migration, and
migrations can't be applied via Render Shell in production today (see "New DB migration" below).
Reusing the existing enum value avoids that migration entirely — the feature now does a full AI
...
column's existing type (`cacheSchemaVersion integer`, no schema change) and the same
`cacheSchemaVersion >= N` comparison pattern the table already used for the old version bumps.

Two legacy `NOT NULL` columns — `source_url` and `attribution` — have no honest per-row meaning
for an AI-generated entry (there is no external source to attribute). Making them nullable would
need a migration, so instead every AI row writes a fixed internal sentinel
(`AI_GENERATED_SOURCE_URL = 'internal:mindlogic-ai-generated'`, `AI_GENERATED_ATTRIBUTION`, both in
`src/services/dictionary/repository.ts`) — clearly not a real URL or license, never disguised as a
Wiktionary source, and **never returned by the public API or shown in the UI.** `audioUrl` is
similarly legacy-only; AI lookups always write `null` and the field was dropped from the public
response entirely.

Cache TTL changed from 30 days to ~10 years (`expiresAt`, set in `ai-lookup.ts`): a word's meaning
doesn't drift day to day, so a fixed short TTL would only re-spend credits regenerating the same
answer. This isn't a true "forever" cache (no background expiry job either) so a row can still be
refreshed — e.g. after a prompt/model change bumps `AI_DICTIONARY_CACHE_SCHEMA_VERSION` — without
one.

**Concurrency and failure handling**, all still inside the same `pg_advisory_xact_lock`-guarded
transaction as before (`getOrRefresh`, `src/services/dictionary/repository.ts`):

- A fresh (unexpired, version ≥ 4) row is served with zero Mindlogic calls.
- 20 concurrent lookups of an uncached word collapse onto exactly one AI call (singleflight via
  the advisory lock) — verified in `tests/integration/dictionary.postgres.test.ts` and, for the
  service layer, `tests/dictionary.test.ts`.
- **Automatic-retry cooldown.** A word whose AI lookup keeps failing must never turn into an
  unbounded automatic retry — every plain page view of that word would otherwise re-spend a
  Mindlogic reservation, forever. `AUTOMATIC_RETRY_COOLDOWN_MS` (5 minutes, same value and
  `shouldSkipAutomaticRetry` mechanism the old Korean-translation-only cooldown used) throttles
  automatic re-attempts; an explicit user retry (`?retry=true`, the dictionary panel's "다시 시도"
  action) always bypasses it. The cooldown timestamp is stored in the existing
  `korean_translation_attempted_at` column (reused as-is, name predates this redesign, no
  migration) — including for a word that has **never** succeeded: a first failure writes a small
  "pending" sentinel row (`cacheSchemaVersion = 1`, satisfying the table's existing
  `meanings_non_empty`/`cache_schema_version_positive` checks, never served as real content) purely
  to carry that cooldown state, so the very next plain lookup of the same still-failing word
  doesn't spend another AI call either.
- A failure is **never** cached as a successful entry. The one exception: if a previously
  successful (version ≥ 4) entry's near-permanent TTL happens to lapse and its background refresh
  fails, the last-known-good result is still served (marked `stale: true`) rather than hard-failing
  a word that already had a real answer.

### Vocabulary saves

Saved vocabulary snapshots `pronunciation` / `definition` / `example` / `koreanTranslations` from
the canonical cached AI entry at save time (never the client), same as before. `senseId` is still
a deterministic SHA-256 hash of `[word, partOfSpeech, definition, example]}` — unchanged shape,
just from AI-generated fields now. `sourceUrl`/`attribution` are still written internally (the
`saved_vocabulary` table has the same legacy `NOT NULL` columns) but, like the lookup response,
are never returned by `GET /api/v1/vocabulary` or `PUT /api/v1/vocabulary/:normalizedWord`.
Article-context validation (UUID must exist, normalized context must appear in the article, the
word must occur at a word boundary) is unchanged.

### Removed: attribution, licensing, and the "AI로 생성될 수 있습니다" notice

Since every meaning is now AI-generated rather than fetched from Wiktionary, the following were
removed from both the API response and the frontend: the `source` field (provider/name/license/
licenseUrl/url), the "Definitions from ... via ..." / "Licensed under ..." footer, and the
"한국어 뜻은 AI로 생성될 수 있습니다." notice (no longer meaningful — the whole entry, not just the
Korean part, is AI-generated now). `audioUrl` and its `<audio>` player were removed from the
dictionary panel for the same reason (no provider ever supplied audio for an AI result).
