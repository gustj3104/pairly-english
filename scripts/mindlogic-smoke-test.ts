...
/**
 * One-shot, approved real smoke test of POST /api/v1/reflections/compare.
 ...
 * the script stops before that POST. A guard file blocks a second run.
 *
 * Run with: MINDLOGIC_MAX_RETRIES=0 pnpm exec tsx scripts/mindlogic-smoke-test.ts
 * (MINDLOGIC_MAX_RETRIES=0 is required — see README's "Procedure for a
 * one-time real smoke test" for why retries must be off here.)
 */

const __dirname = dirname(fileURLToPath(import.meta.url));
const GUARD_FILE = resolve(__dirname, '../.mindlogic-smoke-test-completed.json');
...
function fail(message: string): never {
  console.error(`\n[SMOKE TEST ABORTED — no generative POST was made] ${message}`);
  process.exit(1);
}

async function main(): Promise<void> {
  // --- Guard: refuse a second run outright ---
  if (existsSync(GUARD_FILE)) {
    const record: unknown = JSON.parse(readFileSync(GUARD_FILE, 'utf8'));
    console.error('\n[SMOKE TEST BLOCKED] A completed run already exists:');
  ...
  if (env.MINDLOGIC_MAX_RETRIES !== 0) {
    fail(
      `MINDLOGIC_MAX_RETRIES must be 0 for this smoke test (got ${String(env.MINDLOGIC_MAX_RETRIES)}). Run: MINDLOGIC_MAX_RETRIES=0 pnpm exec tsx scripts/mindlogic-smoke-test.ts`,
    );
  }
  ...
  try {
    ...
    // so a crash below still blocks a second real POST.
    writeFileSync(
      GUARD_FILE,
      JSON.stringify(
        {
...
main().catch((error) => {
  console.error('\n[SMOKE TEST ERROR]', error instanceof Error ? error.message : error);
  process.exit(1);
});
