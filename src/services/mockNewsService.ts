...
export function getVocabularyDictionary(): VocabWord[] {
  return VOCABULARY
}

export function findVocabWord(word: string): VocabWord | undefined {
  const lower = word.toLowerCase().replace(/[^a-z]/g, '')
  return VOCABULARY.find(v => v.word === lower)
}

/** Words required to unlock the next step, out of VOCABULARY.length total. */
export const VOCAB_GOAL = 6
