...
Return only JSON matching the supplied schema, including a topic field that must exactly equal the required topic given in the user message. The required topic is fixed by the editorial calendar for this study date and is not a suggestion — select a story whose central subject is that exact required topic, never a story about a different topic or category, and never a story where the required topic is only a minor or secondary theme, preferably published within the last 72 hours. Avoid political persuasion, sensational crime, sexual violence, graphic violence, and disaster sensationalism. Do not copy long source passages. Clearly distinguish facts from interpretation. Never invent a number, person, quotation, date, publisher, or URL. The sourceUrl's hostname must exactly match one of these domains, or end with one of these suffixes: ${ALLOWED_SOURCE_HOSTS}. Never cite any other outlet — if none of these sources currently cover the required topic, search again until you find one that does, rather than using an outlet outside this list. Base the entire article — title, summary, content, and vocabulary — on exactly one single search result: the one story you are writing about. The sourceUrl must be the exact HTTPS URL of that one search result, never a different result from elsewhere in your search (even from the same outlet or domain) and never a URL you did not actually retrieve as a search result. Write the learning content in English. Provide exactly eight unique vocabulary words. Each vocabulary word must be copied verbatim, in the exact same spelling, inflection, and tense it already has in content — never a lemma, dictionary form, or a different inflection (e.g. if content says "innovating", the vocabulary word must be "innovating", not "innovate") — since it is checked by exact case-insensitive whole-word string match against content, not by meaning. Do not output HTML.`;
